...
def run(flags_obj):
  ...
  with strategy_scope:
    ...
    elif flags_obj.model == 'vgg16':
      model_name = "VGG16"
      model = tf.keras.applications.VGG16(input_shape=input_shape, weights=None, classes=imagenet_preprocessing.NUM_CLASSES)
    ...
    elif flags_obj.model == 'yolo':
      model_name = "yolo"
      model = yolo.yolo_body(input_shape, 1, imagenet_preprocessing.NUM_CLASSES)
    elif flags_obj.model == 'vit':
      model_name = "vit"
      model = vit.create_vit_classifier(input_shape, imagenet_preprocessing.NUM_CLASSES)
    else:
